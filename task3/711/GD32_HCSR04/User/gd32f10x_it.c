...
void USART2_IRQHandler(void)
{
    ...
    if (RESET != usart_interrupt_flag_get(USART2, USART_INT_FLAG_RBNE))
    {
        receive_data = usart_data_receive(USART2);	//接收数据
        //收一个发一个
        usart_data_transmit(USART2, (uint8_t)receive_data);		//发送数据
        while (RESET == usart_flag_get(USART2, USART_FLAG_TBE)); // 发送完成判断
    }
}
